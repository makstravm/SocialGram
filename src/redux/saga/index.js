...
import {
  actionAddComment,
  actionAllPostsCount,
  actionChangeSubscribed,
  actionCreatOrEditPost,
  actionDelLikeComment,
  actionDelLikePost,
  actionEditComment,
  actionGetAboutMe,
  actionGetAllPosts,
  ...
  actionGetAvatar,
  actionGetCommentOne,
  actionGetCommentPost,
  actionGetLikeComment,
  actionGetMyCollection,
  actiongetMyFollowing,
  actionGetMyLikePosts,
  actionGetPostsMyCollection,
  actionGetPostsMyFollowing,
  actionGetPostsTapeCount,
  actionGetProfileData,
  actionGetProfilePagePosts,
  actionGetSubComment,
  actionGetUserFollowers,
  actionLogIn,
  actionOnLikeComment,
  actionOnLikePost,
  actionRegister,
  ...
  actionSubAddComment,
  actionUpsertAboutMe,
  actionUpsertPostsInCollections,
  gql,
} from "../../actions/actionsGetGql";
import {
  actionAboutMeAC,
  actionAboutMeSagaAC,
  actionAddCommentPostInTapeAC,
  actionAddCommentPostOneAC,
  actionAddPostsInPostsTapeAC,
  actionAuthLoginAC,
  actionChangeMyAvatartAC,
  actionChangeStatusLikePostOneAC,
  actionChangeStatusLikePostTapeAC,
  actionClearPostsOneAC,
  actionClearPromise,
  actionEditCommentAC,
  actionFullLogInSagaAC,
  actionLikeCommentAC,
  actionPending,
  actionPostOneDataAC,
  actionProfileDataAC,
  ...
  actionRejected,
  actionResolved,
  actionUpdateCommentAC,
  actionUpdateMyFollowingAC,
  actionUpdateUserFollowersAC,
  actionUpsertCollectionAC,
} from "../../actions/actonsCreators";
...
function* loginWatcher() {
  yield all([
    takeEvery("FULL_LOGIN", logInWorker),
    takeEvery("FULL_REGISTER", fullRegisrterWorker),
  ]);
}
...
function* aboutMeWorker() {
  ...
  if (auth?.payload?.sub?.id) {
    const taskData = yield fork(
      promiseWorker,
      actionGetAboutMe(auth?.payload?.sub?.id)
    );
    const taskCollection = yield fork(
      promiseWorker,
      actionGetMyCollection(auth?.payload?.sub?.id)
    );
    const [dataResult, collectionResult] = yield join([
      taskData,
      taskCollection,
    ]);
    if (dataResult) {
      yield put(
        actionAboutMeAC({ ...dataResult, collection: collectionResult })
      );
    }
  }
}
...
function* postsTapeMyFollowingWorker() {
  const {
    postsTape: { posts, count },
    aboutMe,
  } = yield select();
  const newArrFollowing =
    aboutMe?.following && aboutMe?.following.map((f) => f._id);

  if (posts?.length !== (count ? count : 1)) {
    const taskPosts = yield fork(
      promiseWorker,
      actionGetPostsMyFollowing(posts?.length, [
        ...(newArrFollowing || []),
        aboutMe?._id,
      ])
    );
    const taskCount = yield fork(
      promiseWorker,
      actionGetPostsTapeCount([...(newArrFollowing || []), aboutMe?._id])
    );
...
function* allPostsTapeWorker() {
  ...
  if (posts?.length !== (count ? count : 1)) {
    const taskPosts = yield fork(
      promiseWorker,
      actionGetAllPosts(posts?.length)
    );
...
function* dataProfileWorker({ id }) {
  ...
  if ((posts?.length ? posts?.length : 0) < (count ? count : 1)) {
    if (!count) {
      const taskDataProfile = yield fork(
        promiseWorker,
        actionGetProfileData(id)
      );
      const taskUserPosts = yield fork(
        promiseWorker,
        actionGetProfilePagePosts(id, posts?.length ? posts?.length : 0)
      );
      const taskCountPosts = yield fork(
        promiseWorker,
        actionGetPostsTapeCount([id])
      );

      const [dataProfile, userPosts, countPosts] = yield join([
        taskDataProfile,
        taskUserPosts,
        taskCountPosts,
      ]);
      ...
      const userPosts = yield call(
        promiseWorker,
        actionGetProfilePagePosts(id, posts?.length)
      );
      if (userPosts) {
...
function* changeLikePostWorker({ postId, likeId }) {
  const handlerStatusLikePost = () =>
    likeId ? actionDelLikePost(likeId) : actionOnLikePost(postId);
...
function* changeLikeCommentWorker({ commentId, likeId }) {
  const handlerStatusLikePost = () =>
    likeId ? actionDelLikeComment(likeId) : actionOnLikeComment(commentId);
...
function* subscribeWorker({ userId, followCheck }) {
  ...
  const taskFollowers = yield fork(
    promiseWorker,
    actionGetUserFollowers(userId)
  );
  const taskMyFollowing = yield fork(promiseWorker, actiongetMyFollowing(_id));

  const [{ followers }, { following }] = yield join([
    taskFollowers,
    taskMyFollowing,
  ]);
...
function* sentPostWorker({ images, text, title }) {
  ...
  const result = yield call(
    promiseWorker,
    actionCreatOrEditPost(upSertPostObj)
  );
  if (result) {
...
function* handlerCollectionWorker({ _id, flag }) {
  ...
  yield call(
    promiseWorker,
    actionUpsertPostsInCollections(collection?._id, newCollection)
  );
...
function* loadCollectionWorker() {
  ...
  if (collection?._id) {
    const [{ posts: newResult }] = yield call(
      promiseWorker,
      actionGetPostsMyCollection(collection?._id, dataProfile?.posts?.length)
    );
    if (newResult) {
