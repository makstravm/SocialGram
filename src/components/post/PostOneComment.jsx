...
import { CFieldEditCommentSend, CFieldSubComment } from "../CommentField";
import {
  actionChangeLikeCommentSagaAC,
  actionGetSubCommentSagaAC,
...
const EditMenu = ({ setEditComment }) => (
  <Menu>
    <Menu.Item key="1" onClick={() => setEditComment(true)}>
      <EditOutlined /> Edit
    </Menu.Item>
  </Menu>
);
  ...
  return (
    <>
      ...
      {!editComment ? (
        <Dropdown
          overlay={
            owner?._id === myID && <EditMenu setEditComment={setEditComment} />
          }
          trigger={["contextMenu"]}
        >
...
const CPostCommentText = connect((state) => ({
  ...
  return (
    <>
      <span onClick={() => changeLikeComment(commentId, likeId)}>
        ...
        {!!likes?.length && (
          <span style={{ paddingLeft: 8, cursor: "auto" }}>{likes.length}</span>
        )}
      </span>
      <span onClick={() => setOpen(!open)}>Reply to</span>
      {open && (
        <CFieldSubComment autoFocus={true} id={commentId} setOpen={setOpen} />
      )}
    </>
  );
};

const CPostCommentAction = connect(
  (state) => ({
    myID: state.auth.payload.sub.id || "",
  }),
  {
      ...
      ? comments.map((c) => {
          return (
            <Comment
              ...
              avatar={
                <UserAvatar avatar={c?.owner?.avatar} avatarSize={"35px"} />
              }
              ...
              actions={[
                <CPostCommentAction likes={c.likes} commentId={c._id} />,
              ]}
            >
              <PostOneComments
                comments={c?.answers}
...
export const CPostOneComments = connect(
  (state) => ({
    comments: state?.postOne?.comments || [],
  }),
  { findSubComment: actionGetSubCommentSagaAC }
)(PostOneComments);
